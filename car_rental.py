...
class ConsoleInterface:
    ...
    def _cancel_rental(self):
        ...
        print("=== Отмена аренды ===")
        rentals = [
            rental for rental in self.system.get_user_rentals()
            if rental.status == "active"
        ]
        
        if not rentals:
            print("У вас нет активных аренд для отмены.")
            input("Нажмите Enter для продолжения...")
            return
        ...
        for rental in rentals:
            car = next(car for car in self.system.cars if car.id == rental.car_id)
            print(f"{rental.id}. {car.brand} {car.model} - с {rental.start_date} по {rental.end_date}")
        
        try:
            rental_id = int(input("\nВведите ID аренды для отмены: "))
            
            if self.system.cancel_rental(rental_id):
                print("Аренда успешно отменена.")
            else:
                print("Не удалось отменить аренду. Проверьте ID.")
    ...
    def _add_car(self):
        """Обрабатывает добавление автомобиля (админ)"""
        self._clear_screen()
        ...
        try:
            ...
            daily_price = float(input("Цена за день (руб): "))
            
            if self.system.add_car(brand, model, year, daily_price):
    ...
    def _show_all_rentals(self):
        """Показывает все аренды (админ)"""
        self._clear_screen()
        print("=== Все аренды ===")
        
        if not self.system.rentals:
            print("Нет данных об арендах.")
        else:
            for rental in self.system.rentals:
                car = next((car for car in self.system.cars if car.id == rental.car_id), None)
                car_info = f"{car.brand} {car.model}" if car else "Неизвестный автомобиль"
                print(f"\nАренда #{rental.id}")
                print(f"Пользователь: {rental.username}")
                print(f"Автомобиль: {car_info}")
                print(f"Период: с {rental.start_date} по {rental.end_date}")
                print(f"Стоимость: {rental.total_price:.2f} руб.")
                print(f"Статус: {rental.status}")
...
from car_rental import CarRentalSystem, User, Car, Rental

@pytest.fixture
def system(tmp_path):
    system = CarRentalSystem()
    system.data_dir = tmp_path
    system.cars_file = tmp_path / "cars.json"
    system.rentals_file = tmp_path / "rentals.json"
    system.users_file = tmp_path / "users.json"
    system.cars = []
    system.rentals = []
    system.users = []
    return system
...
def test_register_user(system):
    assert system.register_user("user1", "pass1") is True

def test_register_existing_user(system):
    system.register_user("user1", "pass1")
    assert system.register_user("user1", "pass2") is False

def test_login_user(system):
    system.register_user("user", "123")
    assert system.login("user", "123") is True

def test_add_car_by_admin(system):
    admin = User("admin", "123", role="admin")
    system.users.append(admin)
    system.current_user = admin
    assert system.add_car("Toyota", "Camry", 2022, 5000.0)

def test_rent_car_success(system):
    user = User("test", "123")
    system.users.append(user)
    system.current_user = user
    car = Car(id=1, brand="BMW", model="X5", year=2020, daily_price=1000)
    system.cars.append(car)
    start = (date.today() + timedelta(days=1)).isoformat()
    end = (date.today() + timedelta(days=3)).isoformat()
    assert system.rent_car(1, start, end) == 2000.0

def test_cancel_rental(system):
    user = User("george", "abc", role="customer")
    system.users.append(user)
    system.current_user = user
    rental = Rental(id=1, car_id=1, username="george",
                    start_date="2025-01-01", end_date="2025-01-05",
                    total_price=4000.0, status="active")
    system.rentals.append(rental)
    assert system.cancel_rental(1)
